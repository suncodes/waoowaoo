...
export default function StudioNarrationCanvas({ model, onNavigate }: StudioNarrationCanvasProps) {
  ...
  const hasVoiceLines = model.summary.voiceLines > 0
    ...
    <div className="space-y-4">
      <section className="rounded-lg border border-white/10 bg-[#151613]">
        <StudioStageHeader
          ...
          actions={(
            <div className="flex flex-wrap gap-2">
              <StudioStatusBadge
                status={speechReady ? 'locked' : hasVoiceLines ? 'needs_review' : 'drafting'}
                label={speechReady
                  ? '台词计划就绪'
                  : hasVoiceLines
                    ? '已有台词'
              ...
              <StudioButton variant="secondary" icon="image" onClick={() => onNavigate('storyboard-images')}>
                进入分镜图片
              </StudioButton>
            </div>
          )}
        ...
        <div className="grid gap-3 border-b border-white/10 px-6 py-4 sm:grid-cols-4">
          ...
          <div className="rounded-md border border-white/10 bg-white/[0.03] px-3 py-2">
            <div className="text-xs text-stone-500">镜头计划</div>
            <div className="mt-1 truncate text-sm font-semibold text-stone-100">{model.summary.speechPlanTotal}</div>
          </div>
          <div className="rounded-md border border-white/10 bg-white/[0.03] px-3 py-2">
            <div className="text-xs text-stone-500">音频模式</div>
            <div className="mt-1 truncate text-sm font-semibold text-stone-100">视频原生音频</div>
          </div>
        </div>
