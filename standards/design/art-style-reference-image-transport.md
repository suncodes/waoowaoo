...
- 已从早期 4 类扩展到 14 类，均具备中英文提示词和 JPG 参考图。
...
截至 2026-07-16，`ART_STYLES` 已落地 14 类风格，且 `public/art-styles/` 中保留 14 张 JPG 运行时参考图。
...
| `classic-shanghai-animation` | 合适，新增差异化风格 | 经典国产美术片/上海美术电影质感，与新国风动画、水墨和工笔重彩相近但不重复 | P1 |
| `bikini-bottom-cartoon` | 合适，新增细分卡通风格 | 与美式现代卡通有重叠，但海底荒诞、粗轮廓、高饱和水下配色和电视动画喜剧变形更具体 | P2 |
...
第二轮精修已于 2026-07-16 落地到 `ART_STYLES`，继续保留全部 `value` 和图片路径，重点补充风格边界和反混淆描述：

- `american-comic` 强化为经典美式漫画/超级英雄漫画，增加粗墨线、短缩透视、Ben-Day 半调和漫画分镜张力，明确排除圆润电视卡通、儿童动画和真实摄影。
- `modern-american-cartoon` 强化为现代电视卡通/流媒体动画，强调圆润友好、扁平色块、低中对比和轻喜剧气质，明确排除超级英雄美漫、厚重墨影和半调网点。
- `chinese-comic` 强化为新国风东方幻想动画，突出水墨气韵、工笔线描、现代数字动画、青绿朱砂黛蓝金色配色，明确排除日系赛璐璐、欧美卡通和纯水墨单色画。
- `3d-animation` 与 `cinematic-cg` 做边界拆分：前者偏合家欢风格化 3D 动画，后者偏次世代电影级 PBR/光追/大场面 CG。
- `watercolor-illustration`、`chinese-ink-wash`、`gongbi-heavy-color` 明确区分手绘绘本、水墨留白、工笔重彩三类平面绘画语言。
- `paper-cut-3d` 和 `claymation` 强化材质关键字，分别突出纸张纤维/剪纸层叠和黏土指纹/定格帧感。
- `classic-shanghai-animation` 新增为经典上海美术电影/中国传统美术片方向，强调民族美术、手绘二维动画、胶片颗粒和东方童话氛围。
- `bikini-bottom-cartoon` 新增为海底荒诞美式电视卡通方向，强调水下高饱和配色、粗黑轮廓、扁平色块、喜剧表情和弹性变形。
...
当前实现已将 `appendArtStyleReferenceImage()` 调整为“风格参考图第 1 位”策略。该策略只在用户主动启用 `artStyleReferenceEnabled=true` 且当前风格存在有效 `referenceImage` 时生效；默认关闭时仍然只使用文本风格提示词，不传风格图。

已落地规则：

- 默认策略：`artStyleReferenceEnabled=false` 时不插入风格图，prompt 只使用风格文本。
- 引用顺序：启用后使用 `[styleReferenceImage, ...businessReferenceImages]`，再去重和过滤空值，保证风格图位于第 1 位。
- Prompt 声明：启用后在生成 prompt 中追加“参考图 1 仅用于画面风格、线条、色彩、材质、光影和渲染气质参考；不要复制其中的主体、物品、构图、文字、Logo 或水印。后续参考图仅用于角色、场景或物品一致性。”
- 覆盖链路：项目角色图、项目场景/道具图、分镜图、分镜变体、项目参考图转角色、资产库角色/场景/道具图。
- 资产库策略：资产库仍沿用自动带风格图的行为，但同样执行第 1 位排序和 prompt 声明。
- 图片编辑场景：编辑目标图仍必须保持第一优先级，本轮未把风格图插入图片编辑链路。
...
为解决内置风格仍然无法覆盖全部创作需求的问题，新增项目级“自定义风格”能力。该能力不把用户输入混入 `ART_STYLES` 常量，而是在项目配置中保存自定义风格覆盖项。
...
| 文件/目录 | 修改内容 |
|---|---|
| `src/lib/constants.ts` | 扩展风格注册结构、新增风格、优化内置风格提示词 |
...
- `ART_STYLES` 已扩展为 14 类。
- 14 类风格均具备中英文提示词、预览图和参考图。
- `public/art-styles/` 当前保留 14 张 JPG。
- 风格图已接入角色、场景、分镜和项目级参考图转角色链路，但默认由 `artStyleReferenceEnabled=false` 关闭。
- 风格选择 UI 已改为大图卡片并支持放大预览。
- 参考图传输已不依赖公网 URL。
- 第一轮 prompt-only 优化已完成：保留原有 `value` 和图片路径，仅强化显示名、短标和中英文提示词。
- 火山 Ark A/B 验证显示“新提示词 + 纯文本”更能区分风格；风格参考图已降级为项目级可选增强项。
- 第二轮风格提示词精修已完成：原 12 类风格均补充了反混淆边界。
- 新增 `classic-shanghai-animation` 与 `bikini-bottom-cartoon` 两个内置风格，用于覆盖经典国产美术片和海底荒诞电视卡通场景。
- 风格参考图第 1 位排序策略已完成：启用 `artStyleReferenceEnabled=true` 且存在有效风格图时，风格图排在业务参考图之前。
- Prompt 显式参考图用途声明已完成：启用风格参考图时会声明“参考图 1 仅用于画风/线条/色彩/材质/光影，不复制主体、物品、构图、文字、Logo 或水印”。

当前不继续批量新增风格，优先验证“现有风格质量优化”和新增两类风格的生成效果。
...
- 原 12 类风格优化时，先做 prompt-only A/B；通过后再测试“风格图第 1 位 + 显式引用声明”的效果。
...
12. 当前内置风格的显示名、提示词、参考图语义一致，不出现“名称是水彩、参考图是手绘治愈插画”这类错配。
...
当前已落地 14 类内置风格。下一阶段不继续批量扩张风格数量，先验证新增 `classic-shanghai-animation`、`bikini-bottom-cartoon` 的生成效果，并继续确认显示名、提示词和参考图语义一致。
